...
function PageMain() {
	...
	if(isset($_SESSION['usernameAdmin']) && isset($_SESSION['passwordAdmin'])) {
		...
		if($loggedIn['username']) {
			...
			if($_GET['b'] == 'security') { // Security Admin Tab
				...
				$skin = new skin('admin/pick_of_day'); $page = '';
				// Allowed types
				list($timeline, $message) = $feed->explore(0, 'pick_of_day');
	...
	$menu = array(	''											=> array('admin_menu_dashboard', ''),
					'&b=pick_of_day'							=> array('admin_menu_pick_of_day', ''),
