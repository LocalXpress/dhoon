	...
	if(type == 1) {
		...
		$.ajax({
			...
			success: function(html) {
				console.log(html);
				if(html == 1) {
					location.href = 'index.php?a=profile';
				} else {
					$('.modal-loading').hide();
		...
		$.ajax({
			...
			success: function(html) {
				if(html == 1) {
					alert("An account verification e-mail with login details was sent to "+email+". Please check your spam also.");
					$('.modal-loading').hide();
...
function completeHandler(event) {
	...
	// If the response is successful, remove the form
...
function manageResults(x) {
	if(x == 0) {
		...
		var q = $("#search").val();
		liveLoad('index.php?a=search&q='+q.replace(' ','+')+'&filter=tracks');
	} else if(x == 2) {
		var q = $("#search").val();
...
$(document).ready(function() {
	...
	$("#search").on('keyup', function(e) {
		...
		if(q.substring(0, 1) == '#') {
			var y = 'filter';
			var url = 'tags';
			var full_url = 'index.php?a=explore&'+y+'='+q.replace('#','');
		} else {
			var y = 'q';
			var url = 'people';
			var full_url = 'index.php?a=search&'+y+'='+q.replace(' ','+')+'&filter=tracks';
		}
...
function reload() {
	...
	if(window.location.search.indexOf('a=search') == -1 && window.location.search.indexOf('a=explore&filter=') == -1) {
		$("#search").val('');
	}
...
function prevnext(type) {
	...
	if(type == 3) {
		...
		if(!nextId) {
			...
			if(window.location.search.indexOf('a=stream') > -1 || window.location.search.indexOf('a=explore') > -1 || (window.location.search.indexOf('a=profile') > -1 && window.location.search.indexOf('r=subscriptions') == -1) || (window.location.search.indexOf('a=profile') > -1 && window.location.search.indexOf('r=subscribers') == -1) || (window.location.search.indexOf('a=profile') > -1 && window.location.search.indexOf('r=playlists') == -1) || (window.location.search.indexOf('a=search') > -1 && window.location.search.indexOf('&filter=tracks') > -1)) {
				$('#infinite-load').click();
			}
		}
		return false;
...
$(function() {
	if ($('.search-input').next().attr('title') == 'Connect') {
		$('#explore-button').attr('href', '#');
		$('#explore-button').attr('rel', '');
...
function addToPick(id) {
	$.ajax({
		type: "POST",
		url: baseUrl+"/requests/add_to_pick.php",
		data: "token_id="+token_id+"&id="+id, 
		cache: false,
		success: function(html) {
			var ele = $('#track-song-'+id);
			ele.css('color', 'red');
			ele.text('Remove from Picks of the Day');
			ele.attr('onClick', 'removeFromPick(\''+id+'\')');
		}
	});
}
function removeFromPick(id) {
	$.ajax({
		type: "POST",
		url: baseUrl+"/requests/remove_from_pick.php",
		data: "token_id="+token_id+"&id="+id, 
		cache: false,
		success: function(html) {
			var ele = $('#track-song-'+id);
			ele.css('color', 'green');
			ele.text('Add to Picks of the Day');
			ele.attr('onClick', 'addToPick(\''+id+'\')');
		}
	});
}