...
if(isset($_POST['register'])) {
	...
	if($process > 0) {
		$actUrl = $CONF['url'] . '/requests/active_account.php?token=' . md5($CONF['salt'] . $process) . '&a=' . $process;
		sendMail($_POST['email'], sprintf($LNG['welcome_mail'], $settings['title']), sprintf($LNG['user_created'], $settings['title'], $_POST['username'], $_POST['password'], $CONF['url'], $settings['title'], $actUrl, $actUrl, $CONF['url'], $settings['title']), $CONF['email']);
		$process = 1;
	}
	echo $process;
}
